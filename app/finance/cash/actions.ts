...
async function getCompanyIdOrThrow(supabase: any) {
  ...
  return { company_id: profile.company_id as string, user_id: userRes.user.id as string };
}
...
export async function listCashTransactions(filters: CashFilters = {}): Promise<CashTxRow[]> {
  ...
  let q = supabase
    ...
    .select("id, bank_account_id, tx_type, amount, description, occurred_at, created_at")
    .eq("company_id", company_id);
